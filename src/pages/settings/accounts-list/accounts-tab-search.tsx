...
export const AccountsTabSearch = ({ onSearch, valuesSearch, setOpen, activities }) => {
	...
	const handleOnSubmit = (values: any) => {
		...
		if (["full_name", "email"].includes(searchType) && isEmpty(values.searchByValue)) {
		...
		<Grid container spacing={{ xs: 2, sm: 2 }} alignItems="flex-end">
			<Grid item>
				<Button
					...
					variant="contained"
					sx={{ mr: 2 }}
					disabled={!activities.hasOwnProperty("account_create")}
					onClick={() => activities.hasOwnProperty("account_create") && setOpen(true)}
				>
					Create
				</Button>
				<TextField
