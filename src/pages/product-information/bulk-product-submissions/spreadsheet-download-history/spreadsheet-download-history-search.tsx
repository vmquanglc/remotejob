...
const SearchFileName = ({ onSubmit, searchByValue }) => {
	const formik = useFormik({
		initialValues: {
...
const SearchBy = ({ searchType, onSubmit, searchByValue }) => {
	...
	switch (searchType) {
		case "file_name":
			component = <SearchFileName {...customProps} />;
			break;
	}
	return component;
};

export const SpreadsheetDownloadHistorySearch = ({ onSearch, valuesSearch, selectedRows }) => {
	const [valuesSearchState, setValuesSearchState] = useState({
		status: valuesSearch?.status || "all",
		date: valuesSearch?.date || "all",
		searchBy: valuesSearch?.searchBy || "file_name",
	...
	const searchByOptions = [{ label: "File name", value: "file_name", status: false }];
	const optionsStatus = [
		{ label: "All status", value: "all", status: false },
		{ label: "In Progress", value: 1, status: false },
		{ label: "Action required", value: 2, status: false },
		{ label: "Done", value: 3, status: false },
		{ label: "Failed", value: -1, status: false },
	];
	const optionsDates = [
		{ label: "All date", value: "all", status: false },
	...
	const handleOnSubmit = (values: any) => {
		...
		cloneValues.status = valuesSearchState.status;
		cloneValues.date = valuesSearchState.date;
		if (["file_name"].includes(valuesSearchState.searchBy) && isEmpty(values.searchByValue)) {
		...
		<Grid container spacing={{ xs: 2, sm: 2 }} alignItems="flex-end">
			<Grid item>
				<TextField
					...
					SelectProps={{
						multiple: false,
						value: valuesSearchState.status,
						onChange: handleOnChange,
					}}
					sx={{ width: 180 }}
				>
					{map(optionsStatus, ({ label, value, status }) => (
